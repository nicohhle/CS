...
class WordLadder {
  ...
  private:
    // checks to see if the word is a letter off from it's previous
    bool letterOff(const string &old, const string &compare);
    
    // deals with the printing to output file instead of doing it multiple times
    void print(stack<string> &s, ofstream &out);
};